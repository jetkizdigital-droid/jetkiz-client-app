...
void main() {
  ...
  test('pending payment is readable only by the same authenticated user', () async {
  ...
  test('logout clears pending payment recovery together with auth tokens', () async {
    FlutterSecureStorage.setMockInitialValues(<String, String>{
...
String _jwtFor(String userId) {
  final header = base64Url.encode(utf8.encode(jsonEncode(<String, dynamic>{
    'alg': 'none',
    'typ': 'JWT',
  }))).replaceAll('=', '');
