...
class _SmsCodePageState extends State<SmsCodePage> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // Android can keep the FocusNode marked as focused while hiding the IME
    // after the user opens WhatsApp to read the OTP. Explicitly re-show the
    // text input on resume so every OTP cell remains usable.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && !_isSubmitting) _focusCodeInput();
    });
  }
  ...
  Future<void> _resendCode() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _errorText = _resendErrorMessage(error));
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildCodeBox({required int index, required String digits}) {
    final hasValue = digits.length > index;
    final isActive = digits.length == index && !_isSubmitting;
  ...
  Widget _buildOtpInput(String digits) {
    return GestureDetector(
      ...
      child: Stack(
        children: [
          ...
          Positioned.fill(
            child: Opacity(
              // Keep a real-size editable control in the semantics/hit-test tree.
              // Zero-sized hidden TextFields can lose the Android IME after resume.
              opacity: 0.01,
              child: TextField(
